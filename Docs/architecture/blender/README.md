...
- 修改 Scene 摄影机、World、Light `hide_render`、Collection `hide_render`、LayerCollection `exclude`、输出路径、引擎、图像设置、色彩管理、Workbench 参数、对象显示色或临时数据块时，先登记原值，再修改；恢复顺序与依赖相反。用户在配对列表中主动选择组合属于预览操作，经统一 Environment Adapter 应用，不归渲染事务回滚。
