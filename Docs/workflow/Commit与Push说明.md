...
3. 构建同版本 Extension 与 Legacy ZIP，运行 `scripts/release_gate.py`，确认两包功能文件逐字节一致。
4. 完成目标 Blender 版本、安装态、渲染、取消/失败和状态恢复验证。
5. README 顶部按钮、正文安装链接和文件名全部指向本次版本，禁止残留旧版本下载 URL。
6. 创建符合规范的 commit 并 push 主分支；创建同版本 Git tag/GitHub Release，上传两个 ZIP 和校验值。
7. 运行 `python scripts/release_gate.py --verify-online`，要求最新版 Release 的两个链接均返回成功。
...
报告至少包含：commit hash、tag/Release 版本、Extension/Legacy 资产名、两包校验结果、在线下载验证结果、测试摘要以及工作区是否干净。
