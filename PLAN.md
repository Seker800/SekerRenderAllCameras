...
2. 所有结果保存到 `.blend` 文件同级的 `RenderOutput/批次编号/`。
3. 同一轮任务使用统一三位数批次前缀，例如 `001`。
...
9. 批次编号以文件系统为唯一事实来源，不依赖 `.blend` 内属性持久化。
...
- `批次前缀`：默认显示文件系统中第一个可用编号，例如 `001`。
- `完成后显示下一编号`：默认开启，只影响界面显示，不写入 Scene。
- `输出 Alpha`：默认关闭。
- `输出 Object ID`：默认关闭。
- `已有文件处理方式`：
  - 使用下一可用批次，默认且推荐；
...
2. 使用 `bpy.path.abspath()` 解析 Blender 的 `//` 相对路径。
3. 任务启动时从用户输入编号开始扫描。
4. 通过“目录不存在才创建”的原子操作占用批次编号；如果创建冲突，继续尝试下一编号。
5. 批次目录创建成功后立刻写入 `.inprogress` 和初始清单。
6. 正常完成后删除 `.inprogress`。
7. 取消或失败时保留 `.inprogress` 或改名为 `.incomplete`，便于识别未完整批次。
8. 不通过 Scene Property 保存“下一个编号”；再次打开文件时重新扫描目录。

### 6.1 批次编号

- 输入只接受非负整数。
- 显示时最少补足三位：`1 → 001`，`12 → 012`。
- 超过三位不截断：`1000 → 1000`。
- 本轮开始后编号冻结。
- 正常完成后界面显示下一个可用编号。
- 取消、失败或关闭 Blender 后，下次仍通过目录扫描获得正确编号。
...
```text
{批次}_{Blend文件名}_{摄影机名}_{通道}_{关键参数}.{扩展名}
```

示例：

```text
001_客厅方案_Camera_Front_Beauty_3840x2160_Cycles_S256.png
001_客厅方案_Camera_Front_Alpha_3840x2160.png
001_客厅方案_Camera_Front_ObjectID_3840x2160_Object.png
001_客厅方案_RenderInfo.json
001_客厅方案_ObjectID.json
```
...
当前帧不默认进入文件名，因为一轮任务已有唯一批次目录；帧号必须写入清单。
...
```text
001_客厅方案_RenderInfo.json
001_客厅方案_ObjectID.json    # 仅开启 ID 时
.inprogress                   # 未完成标记
```
...
- JSON 更新失败：视为批次级严重错误，停止新渲染并清理，因为任务已无法可靠追踪。

### 14.3 已有文件

- 默认通过新批次目录避免冲突。
- “跳过”必须逐文件记录 `skipped_existing`。
- “覆盖”只能覆盖本轮明确计算出的文件，不能删除批次目录中的其他内容。
- 写图片失败时不得删除之前已存在且未成功替换的文件；能够使用临时文件的输出应采用临时写入后替换。
...
- `storage.py`：路径、原子批次占用、冲突策略和原子文件操作。
...
2. 所有输出位于 `.blend` 同级 `RenderOutput/批次号/`。
3. 本轮文件统一使用同一批次前缀。
